...
(function ($) {
    ...
    var phpDate = function (e, g) {
        var h = this;
        var k, f;
        var l = ['Sun', 'Mon', 'Tues', 'Wednes', 'Thurs', 'Fri', 'Satur', 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
        var m = /\\?(.?)/gi;
        var o = function (t, s) {
            return f[t] ? f[t]() : s
        };
        ...
        f = {
            d: function () {
                ...
                var H = k.getUTCHours() * 36e2;
                var i = k.getUTCMinutes() * 60;
                var s = k.getUTCSeconds();
                ...
                var b = k.getTimezoneOffset();
                var a = Math.abs(b);
                return (b > 0 ? '-' : '+') + p(Math.floor(a / 60) * 100 + a % 60, 4)
            }, P: function () {
                var O = f.O();
        ...
        this.date = function (a, b) {
            h = this;
            k = (b === undefined ? new Date() : (b instanceof Date) ? new Date(b) : new Date(b * 1000));
            return a.replace(m, o)
        };
        return this.date(e, g)
    };
    var strtotime = function (e, f) {
        var g, match, today, year, date, days, ranges, len, times, regex, i, fail = false;
        if (!e) {
            return fail
        }
        e = e.replace(/^\s+|\s+$/g, '').replace(/\s{2,}/g, ' ').replace(/[\t\r\n]/g, '').toLowerCase();
        match = e.match(/^(\d{1,4})([\-\.\/\:])(\d{1,2})([\-\.\/\:])(\d{1,4})(?:\s(\d{1,2}):(\d{2})?:?(\d{2})?)?(?:\s([A-Z]+)?)?$/);
        ...
        function process(a) {
            var b = a.split(' '), type = b[0], range = b[1].substring(0, 3), typeIsNumber = /\d+/.test(type), ago = b[2] === 'ago', num = (type === 'last' ? -1 : 1) * (ago ? -1 : 1);
            if (typeIsNumber) {
        ...
        match = e.match(new RegExp(regex, 'gi'));
        if (!match) {
            return fail
        }
    ...
    var Dropdown = function (element, name, filter) {
        ...
        var MESOUR_TRUE = '-mesour-bool-1';
        var MESOUR_FALSE = '-mesour-bool-0';
        var MESOUR_NULL = '-mesour-null';
        ...
        var getRealValue = function (variable) {
            if(variable === MESOUR_NULL) {
                return null;
            } else if (variable === MESOUR_TRUE) {
                return true;
            } else if(variable === MESOUR_FALSE) {
                return false;
            }
            return variable;
        };
        var translateVariable = function (variable) {
            if(variable === null) {
                return '<i>'+mesour.dataGrid.translates['empty']+'</i>';
            } else if (variable === true) {
                return '<i>'+mesour.dataGrid.translates['true']+'</i>';
            } else if(variable === false) {
                return '<i>'+mesour.dataGrid.translates['false']+'</i>';
            }
            return variable;
        };
            ...
            for (var x = 0; x < gridData.length; x++) {
                if (!values[gridData[x][name]]) {
                    values[fixVariable(gridData[x][name])] = {
                        ...
                        translated: translateVariable(gridData[x][name]),
                        keys: [x]
                    };
                } else {
        ...
        this.getValues = function (valType) {
            ...
            if (!valType) {
                ...
                if (!val[valType]) {
                    ...
                    return getRealValue(val[valType]);
                }
            }
        };
