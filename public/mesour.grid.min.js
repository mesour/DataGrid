...
var mesour=!mesour?{}:mesour;if(!mesour.filter){throw new Error('Widget mesour.filter is not created. First create mesour.filter widget.');}mesour.filter.applyFilter=function(a,b){b=$.parseJSON(b);b={filterData:b};var c=mesour.core.createLink(a,'applyFilter',b,true);$.post(c[0],c[1])};mesour.filter.Filter=function(f,g){var h=this;var j={},valuesInput=g,dropDownLink=g.attr('data-dropdown-link'),iconPrefix=g.attr('data-icon-prefix'),icons=jQuery.parseJSON(g.attr('data-icons')),resetButton=$('.full-reset[data-filter-name="'+f+'"]');this.apply=function(){mesour.filter.applyFilter(f,valuesInput.val())};this.getIconClass=function(a){if(!icons[a]){throw new Error('Icon type '+a+' not exists.');}return iconPrefix+icons[a]};this.getIconPrefix=function(){return iconPrefix};this.getDropdowns=function(){return j};this.getName=function(){return f};this.getDropDownLink=function(){return dropDownLink};this.getFilterModal=function(){return m};this.closeAll=function(a){for(var x in j){if(!j.hasOwnProperty(x)){continue}j[x].update()}g.find('.dropdown').each(function(){if(!a||$(this)[0]!==a[0]){$(this).removeClass('open');mesour.cookie(f+'-'+$(this).attr('data-filter'),0)}})};var l=$.parseJSON(valuesInput.attr('data-mesour-data'));this.getData=function(){return l};this.getPhpDateFormat=function(){return valuesInput.attr('data-mesour-date')};this.getJsDateFormat=function(){return valuesInput.attr('data-mesour-js-date')};var m=$('.mesour-filter-modal.modal-dialog');if(!m.is('*')){m=$('<div class="mesour-filter-modal modal-dialog"> <div class="modal-content"> <div class="modal-header"> <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button> <h4 class="modal-title">Custom filter</h4> </div> <div class="modal-body"> <form class="form-inline"> <p>Show rows where:</p> <div class="form-group"> <label class="sr-only"></label> <select class="form-control filter-how-1"> <option></option> <option value="equal_to">Equal to</option> <option value="not_equal_to">Not equal to</option> <option value="bigger">Is greater than</option> <option value="not_bigger">Is no greater than</option> <option value="smaller">Is smaller than</option> <option value="not_smaller">Is no smaller than</option> <option value="start_with">Starts with</option> <option value="not_start_with">Not starts with</option> <option value="end_with">Ends with</option> <option value="not_end_with">Not ends with</option> <option value="equal">Contains</option> <option value="not_equal">Not contains</option> </select> </div> <div class="form-group"> <label class="sr-only">Value</label> <div class="input-group date filter-datepicker1"> <input type="text" class="form-control filter-value-1" data-date-format="'+h.getJsDateFormat()+'" placeholder="Value"> <span class="input-group-addon"> <span class="'+h.getIconClass(mesour.filter.ICON_CALENDAR)+'"></span> </span> </div> </div> <br> <div class="form-group grid-operators"> <input type="radio" name="operator" class="filter-operator-and" value="and" checked="checked"> <label>and</label> <input type="radio" name="operator" class="filter-operator-or" value="or"> <label>or</label> </div> <br> <div class="form-group"> <label class="sr-only"></label> <select class="form-control filter-how-2"> <option></option> <option value="equal_to">Equal to</option> <option value="not_equal_to">Not equal to</option> <option value="bigger">Is greater than</option> <option value="not_bigger">Is no greater than</option> <option value="smaller">Is smaller than</option> <option value="not_smaller">Is no smaller than</option> <option value="start_with">Starts with</option> <option value="not_start_with">Not starts with</option> <option value="end_with">Ends with</option> <option value="not_end_with">Not ends with</option> <option value="equal">Contains</option> <option value="not_equal">Not contains</option> </select> </div> <div class="form-group"> <label class="sr-only">Value</label> <div class="input-group date filter-datepicker2"> <input type="text" class="form-control filter-value-2" data-date-format="'+h.getJsDateFormat()+'" placeholder="Value"> <span class="input-group-addon"> <span class="'+h.getIconClass(mesour.filter.ICON_CALENDAR)+'"></span> </span> </div> </div> </form> </div> <div class="modal-footer"> <button type="button" class="btn btn-primary btn-sm save-custom-filter">Ok</button> <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">Storno</button> </div> <input type="hidden" data-name=""> </div> <!-- /.modal-content --> </div> <!-- /.modal-dialog -->');$('body').append(m);m.find('[aria-hidden="true"], [data-dismiss="modal"]').on('click.custom-filter',function(e){e.preventDefault();$(this).closest('.modal-dialog').fadeOut()});m.find('.save-custom-filter').on('click',function(){var a=m.find('[data-name]').val(),internalValues={how1:m.find('.filter-how-1').val(),how2:m.find('.filter-how-2').val(),val1:m.find('.filter-value-1').val(),val2:m.find('.filter-value-2').val(),operator:m.find('input[name="operator"]:checked').val()};if(internalValues.how1.length===0){alert('Please select some value in first select.');m.find('.filter-how-1').focus();return}if(internalValues.val1.length===0){alert('Please insert some value for first text input.');m.find('.filter-value-1').focus();return}if(internalValues.how2.length!==0&&internalValues.val2.length===0){alert('Please insert some value for second input.');m.find('.filter-value-2').focus();return}j[a].setValues(internalValues,'custom');j[a].setValues(j[a].getType()!=='date'?'text':'date','type');$(this).closest('.modal-dialog').fadeOut();j[a].update();j[a].save();j[a].getFilter().apply()})}this.getValues=function(a){var b=valuesInput.val();b=b.length===0?{}:$.parseJSON(b);if(!a){return b}else{if(!b[a]){return{}}else{return b[a]}}};this.setValues=function(a,b){var c=valuesInput.val().length>0?$.parseJSON(valuesInput.val()):{};if(c instanceof Array){c={}}c[b]=a;valuesInput.val(JSON.stringify(c))};this.refreshPriorities=function(){var a=h.getValues();var b={};for(var x in a){if(!a.hasOwnProperty(x)){continue}b[a[x].priority]=x}var c=[];for(var k in b){if(b.hasOwnProperty(k)){c.push(k)}}c.sort();var d=1;for(var i=0;i<c.length;i++){k=c[i];if(a[b[k]].priority){a[b[k]].priority=d;d++}}valuesInput.val(JSON.stringify(a))};this.generateNextPriority=function(){h.refreshPriorities();var a=h.getValues();var b=[];for(var x in a){if(!a.hasOwnProperty(x)){continue}b.push(a[x].priority)}if(b.length>0){var c=1;for(var y=0;y<b.length;y++){if(b[y]>c){c=b[y]+1}else if(b[y]===c){c++}}return c}else{return 1}};this.filterData=function(a,b){var c=h.getData(),output=[];for(var x in c){if(!c.hasOwnProperty(x)){continue}if(b.indexOf(c[x][a])!==-1){output.push(c[x])}}return output};this.filterCheckers=function(){var a=h.getValues(),usedPriorities={};for(var z in a){if(!a.hasOwnProperty(z)){continue}usedPriorities[a[z].priority]=z}var b=[];for(var k in usedPriorities){if(usedPriorities.hasOwnProperty(k)){b.push(k)}}b.sort();var c={},newData=h.getData();for(var i=0;i<b.length;i++){k=b[i];c[usedPriorities[k]]=true;if(!j[usedPriorities[k]]){continue}j[usedPriorities[k]].destroy();j[usedPriorities[k]].create(newData,true);j[usedPriorities[k]].update();if(a[usedPriorities[k]].checkers&&a[usedPriorities[k]].checkers.length>0)newData=h.filterData(usedPriorities[k],a[usedPriorities[k]].checkers)}for(var x in j){if(!j.hasOwnProperty(x)){continue}var d=j[x];if(c[d.getName()])continue;j[x].destroy();j[x].create(newData,true);j[x].update()}};resetButton.on('click',function(e){e.preventDefault();$.each(h.getDropdowns(),function(a,b){b.unsetValues('custom');b.unsetValues('priority');b.unsetValues('checkers');b.update();b.getFilter().filterCheckers()});h.apply()});$('.dropdown[data-filter-name="'+f+'"]').each(function(){var a=$(this),name=a.attr('data-filter');j[name]=new mesour.filter.DropDown(a,name,h);a.data('grid-filter-dropdown',j[name])});h.filterCheckers();if($.fn.bootstrapDatetimepicker){$('.filter-datepicker1, .filter-datepicker2').bootstrapDatetimepicker({pickTime:false})}};
/* src/mesour.filter.DropDown.js */
var mesour=!mesour?{}:mesour;if(!mesour.filter){throw new Error('Widget '+mesour.filter+' is not created. First create mesour.filter widget.');}mesour.filter.applyDropDown=function(a,b,c){if(c!==''){var d=c.id;var e=c.opened;mesour.cookie(a+'-'+d,e)}};mesour.filter.DropDown=function(E,F,G){var H=this;var I=function(a){if(a===null){return mesour.filter.VALUE_NULL}else if(a===true){return mesour.filter.VALUE_TRUE}else if(a===false){return mesour.filter.VALUE_FALSE}return a};this.fixVariable=function(a){return I(a)};this.translateVariable=function(a){if(a===null){return'<i>'+mesour.filter.translations['empty']+'</i>'}else if(a===true){return'<i>'+mesour.filter.translations['true']+'</i>'}else if(a===false){return'<i>'+mesour.filter.translations['false']+'</i>'}else if(L[a]){return L[a]}return a};this.fixDropDownPosition=function(){var a=$(window).width();var b=$(this).width();var c=$(this).closest('.dropdown-menu').offset();if(a-(c.left+b)<250){$(this).addClass('pull-left')}else{$(this).removeClass('pull-left')}};var J=E.attr('data-type');var K=E.find('[data-translates]');var L=K.is('*')?jQuery.parseJSON(K.val()):[];E.find('.dropdown-submenu').on('mousemove',this.fixDropDownPosition);var M,checkers,cookieName=G.getName()+'-'+F+'-checkers',mouseIn=false;var N=function(){};var O=function(){var a=E.find('.box-inner').find('ul');a.find('li:not(.all-select-li):not(.all-select-searched-li)').remove()};this.getCookieName=function(){return cookieName};this.destroy=function(){O()};this.create=function(a,b){N(a,b)};var P=function(a){mesour.filter.applyDropDown(G.getName(),G.getDropDownLink(),a)};N=function(d,e){d=!d?G.getData():d;if(!d)return;var f={};for(var x=0;x<d.length;x++){if(typeof d[x][F]==='undefined'){throw new Error('MesourFilterDropDownException: Column "'+F+'" does not exists in data.');}if(!f[d[x][F]]){f[I(d[x][F])]={val:I(d[x][F]),translated:H.translateVariable(d[x][F]),keys:[x]}}else{f[d[x][F]].keys.push(x)}}if(!J){var g=E.find('.box-inner').find('ul');for(var y in f){if(!f[y].val&&Number(f[y].val)!==0)continue;var h=$('<li>'),id=F+((f[y].val&&typeof f[y].val.replace==='function')?f[y].val.replace(' ',''):f[y].val);h.append('<input type="checkbox" class="checker" data-value="'+f[y].val+'" id="'+id+'">');h.append('&nbsp;');h.append('<label for="'+id+'">'+f[y].translated+'</label>');g.append(h)}}else if(J==='date'){var j=[],months={},special={};for(var y in f){if(!f[y].val)continue;var k=isNaN(f[y].val);if(f[y].val){if(f[y].val===mesour.filter.VALUE_NULL||f[y].val===mesour.filter.VALUE_TRUE||f[y].val===mesour.filter.VALUE_FALSE){special[f[y].val]=f[y];continue}}var l=k?mesour.core.strtotime(f[y].val):f[y].val;var m=mesour.core.phpDate('Y',l);var n=mesour.core.phpDate('n',l);var o=mesour.core.phpDate('j',l);if(j.indexOf(m)===-1){j.push(m)}if(!months[m]){months[m]={};months[m]['months']=[];months[m]['days']={}}if(months[m]['months'].indexOf(n)===-1){months[m]['months'].push(n)}if(!months[m]['days'][n]){months[m]['days'][n]=[]}if(months[m]['days'][n].indexOf(o)===-1){months[m]['days'][n].push(o)}}j.sort(function(a,b){return b-a});var g=E.find('.box-inner').find('ul');for(var i in special){if(!special.hasOwnProperty(i)){continue}var h=$('<li>'),id=F+((special[i].val&&typeof special[i].val.replace==='function')?special[i].val.replace(' ',''):special[i].val);h.append('<input type="checkbox" class="checker" data-value="'+special[i].val+'" id="'+id+'">');h.append('&nbsp;');h.append('<label for="'+id+'">'+special[i].translated+'</label>');g.append(h)}var p=mesour.cookie(cookieName),checkersInfo=p?jQuery.parseJSON(p):{};for(var a in j){if(!j.hasOwnProperty(a)){continue}var q='year-'+j[a];var r=checkersInfo[q]?true:false;var s=$('<li>');s.append('<span class="'+G.getIconClass(r?mesour.filter.ICON_MINUS:mesour.filter.ICON_PLUS)+' toggle-sub-ul" data-name="'+q+'"></span>');s.append('&nbsp;');s.append('<input type="checkbox" class="checker">');s.append('&nbsp;');s.append('<label>'+j[a]+'</label>');s.append('<span class="close-all">(<a href="#">Close all</a>)</span>');var t=$('<ul class="toggled-sub-ul">');s.append(t);if(r){t.show();s.find('.close-all').show()}months[j[a]].months.sort(function(a,b){return a-b});var n=months[j[a]].months;for(var b in n){var u='month-'+j[a]+'-'+n[b];var v=checkersInfo[u]?true:false;var w=$('<li>');w.append('<span class="'+G.getIconClass(v?mesour.filter.ICON_MINUS:mesour.filter.ICON_PLUS)+' toggle-sub-ul" data-name="'+u+'"></span>');w.append('&nbsp;');w.append('<input type="checkbox" class="checker">');w.append('&nbsp;');w.append('<label>'+mesour.filter.translations.months[n[b]]+'</label>');t.append(w);var z=$('<ul class="toggled-sub-ul">');w.append(z);if(v){z.show()}months[j[a]].days[n[b]].sort(function(a,b){return a-b});var A=months[j[a]].days[n[b]];for(var c in A){var B=mesour.core.strtotime(j[a]+'-'+n[b]+'-'+A[c]);var C=k?mesour.core.phpDate(G.getPhpDateFormat(),B):B;var D=$('<li>');D.append('<span class="'+G.getIconPrefix()+'">&nbsp;</span>');D.append('<input type="checkbox" class="checker" data-value="'+C+'">');D.append('&nbsp;');D.append('<label>'+A[c]+'</label>');z.append(D)}}g.append(s)}}if(e){checkers=new mesour.filter.Checkers(H)}};N();this.getName=function(){return F};this.getType=function(){return!J?'text':J};this.getElement=function(){return E};this.getValues=function(a){var b=G.getValues(F);if(!a){return b}else{if(!b[a]){return{}}else{return b[a]}}};this.setValues=function(a,b){var c=G.getValues(F);c[b]=a;G.setValues(c,F)};this.unsetValues=function(a){var b=G.getValues(F);delete b[a];G.setValues(b,F)};this.getFilter=function(){return G};M=new mesour.filter.CustomFilter(this);checkers=new mesour.filter.Checkers(this);this.update=function(){var a=H.getValues(),toggle_button=E.find('.dropdown-toggle'),menu=E.find('.dropdown-menu'),first_submenu=menu.children('.dropdown-submenu');toggle_button.find('[data-filter-icon="check"]').hide();first_submenu.find('[data-filter-icon]').closest('button').hide();E.removeClass('active-item').removeClass('active-checkers');if(a){if(a.custom&&a.custom.operator){toggle_button.find('[data-filter-icon="check"]').show();E.addClass('active-item');first_submenu.find('[data-filter-icon]').closest('button').show()}if(a.checkers&&typeof a.checkers[0]!=='undefined'){toggle_button.find('[data-filter-icon="check"]').show();E.addClass('active-checkers');for(var x=0;x<a.checkers.length;x++){checkers.check(a.checkers[x])}}}};this.toggle=function(){if(H.isOpened()){H.close()}else{H.open()}};this.isOpened=function(){return E.hasClass('open')};this.open=function(){G.closeAll(E);E.addClass('open');P({id:H.getName(),opened:1})};this.close=function(){H.update();E.removeClass('open');P({id:H.getName(),opened:0})};E.on({mouseenter:function(){mouseIn=true},mouseleave:function(){mouseIn=false}});$('.mesour-filter-modal').on({mouseenter:function(){mouseIn=true},mouseleave:function(){mouseIn=false}});$('html').on('click.filter-el-'+F,function(){if(H.isOpened()&&!mouseIn){H.close()}});E.children('button').on('click',function(e){e.preventDefault();G.closeAll(E);H.toggle(E)});E.find('.reset-filter').on({click:function(){H.unsetValues('custom');H.update();H.save();G.apply()},mouseenter:function(){$(this).removeClass('btn-success').addClass('btn-danger').find('[data-filter-icon]').hide().filter('[data-filter-icon="reset"]').show()},mouseleave:function(){$(this).removeClass('btn-danger').addClass('btn-success').find('[data-filter-icon]').hide().filter('[data-filter-icon="has-custom"]').show()}});E.find('.close-filter').on('click',function(e){e.preventDefault();H.update();H.close()});this.save=function(){var a=checkers.getChecked();if(a.length>0){H.setValues(H.getFilter().generateNextPriority(),'priority');H.setValues(a,'checkers');H.setValues(J!=='date'?'text':'date','type')}else{H.unsetValues('priority');H.unsetValues('checkers')}};this.update()};
/* mesour.selection.js */
...
/* mesour.grid.core.js */
var mesour=!mesour?{}:mesour;mesour.grid=!mesour.grid?{}:mesour.grid;(function($){var c=function(){var a=$('.mesour-datagrid');a.find('.only-buttons').each(function(){$(this).closest('td').addClass('actions-column')});a.find('[data-mesour-enabled-filter="1"]').find('.selection-dropdown').each(function(){$(this).before('<span class="fake-header">&nbsp;</span>')});$('[data-mesour-toggle="tooltip"]').tooltip()};c();$(document).on('ready',c);$(window).on('ajaxComplete',c);$(window).on('ajaxError',c);var d=function(a){var b=this;this.items={}};mesour.core.createWidget('grid',new d())})(jQuery);
/* mesour.grid.sortable.js */
var mesour=!mesour?{}:mesour;if(!mesour.selection){throw new Error('Widget mesour.selection is not created. First create mesour.selection widget.');}mesour.gridSortable=!mesour.gridSortable?{}:mesour.gridSortable;(function($){var h=function(f){var g=this;this.sendData=function(a,b){var c=mesour.core.createLink(a,'sortData',b,true);$.post(c[0],c[1]).complete(mesour.core.redrawCallback)};this.create=function(){$('['+f.attributeName+']').children('tbody').sortable({disableNesting:'no-child',forcePlaceholderSize:true,autoScroll:true,handle:'a.handler',listType:'tbody',helper:'clone',items:'> tr:not(.no-sort)',maxLevels:$('a#sort_href').attr("title"),opacity:.6,placeholder:'placeholder',revert:250,tabSize:25,tolerance:'pointer',toleranceElement:'> td',stop:function(a,b){var c=b.item.attr('id').split('-');var d=$(this);var e={data:d.sortable('serialize'),item:c[c.length-1]};g.sendData(d.closest('['+f.attributeName+']').attr(f.attributeName),e)},start:function(a,b){var c;$(this).find('.no-sort').remove();if(b.placeholder.is('tr')&&c==undefined){var x=0;$('thead tr th',$(a.currentTarget).closest('table')).each(function(){x++;$('td:nth-child('+x+')',b.helper).css('width',$(this).width())});$(b.placeholder).find('td').css('height',$(b.placeholder).next('tr').outerHeight());c=true}else if(c==true){c=false}}})}};mesour.core.createWidget('gridSortable',new h({attributeName:'data-mesour-sortable'}));mesour.on.live('grid-sortable-links',mesour.gridSortable.create)})(jQuery);
...
var mesour=!mesour?{}:mesour;mesour.gridEditable=!mesour.gridEditable?{}:mesour.gridEditable;(function($){var q=false,isFloat=function(n){return n===+n&&n!==(n|0)},isInteger=function(n){return n===+n&&n===(n|0)};var r=function(o){var p=this;this.sendData=function(a,b){var c=mesour.core.createLink(a,'editCell',b,true);return $.post(c[0],c[1]).complete(mesour.core.redrawCallback)};this.create=function(){$('.mesour-datagrid').find('['+o.attributeName+']').each(function(){var m=$(this),editable_link=m.attr(o.attributeName),mouse_in=false,closeAllEditables=function(){$('[data-editable]').each(function(){if($(this).find('input,select').is('*')){var a=$(this).find('input,select').attr('data-old-value');$(this).html(a)}});q=false},isMouseIn=function(){return mouse_in};var n=function(){if(window.getSelection)window.getSelection().removeAllRanges();else if(document.selection)document.selection.empty();if(q)return;q=true;var c=$(this),gridName=$(this).closest('[data-mesour-grid]').attr('data-mesour-grid');var d=$(this);d.width(d.width());var f=d.closest('tr');if(!f.is('*')){f=d.closest('li')}var g=f.attr('id').split('-'),line_id=g[g.length-1],column_name=d.attr('data-editable'),related=d.attr('data-editable-related'),type=d.attr('data-editable-type'),pickTime=d.attr('data-editable-pickTime')==1,separator=d.attr('data-separator'),unit=d.attr('data-unit'),date_format=d.attr('data-date-format'),mouse_in=true,old_value=d.html(),keyUpCallback=function(e,a){if(e.keyCode===13||a){var b=getValidValue();if(type==='date'){l.data('DateTimePicker').destroy()}if(b!==false){p.sendData(editable_link,{'data':{lineId:line_id,columnName:column_name,oldValue:old_value,dataValue:d.attr('data-value'),newValue:l.val()}}).complete(function(){d.html(l.is('select')?l.find('option:selected').text():l.val());q=false}).error(function(){closeAllEditables()})}}else if(e.keyCode===27){closeAllEditables()}};var h='<input class="editable-input" type="text" data-old-value="'+old_value+'">';if(related){h=$('<select class="editable-input" data-old-value="'+old_value+'"></select>');h.on('change',function(){keyUpCallback.call(h,{keyCode:13})});if(mesour.grid.items[gridName]&&mesour.grid.items[gridName].relations[related]){var j=mesour.grid.items[gridName].relations[related];for(var i in j){var k=$('<option value="'+i+'">'+j[i]+'</option>');h.append(k);if(j[i]===old_value){k.prop('selected',true)}}}}var l=$(h),getValidValue=function(){if(type==='number'){var a=Number(l.val().replace(',','.'));if(isInteger(a)||isFloat(a)){return l.val()}else{alert('Value of this column must be a number.');return false}}return l.val()};l.width(d.innerWidth());l.on({'keyup':keyUpCallback,'mouseenter':function(){mouse_in=true},'mouseleave':function(){mouse_in=false}});$(document).on('mouseenter','.ui-datepicker',function(){mouse_in=true});$(document).on('mouseleave','.ui-datepicker',function(){mouse_in=false});isMouseIn=function(){return mouse_in};$(this).html(l);switch(type){case'number':l.val($.trim(old_value.replace(separator,'').replace(unit,'')));break;case'date':l.val(!old_value||old_value==='-'?'':old_value);l.bootstrapDatetimepicker({format:date_format,useSeconds:true,pickTime:pickTime,focusOnShow:false,hide:function(){l.focus()},useCurrent:false});break;default:if(l.is('input')){l.val(old_value)}}l.focus()};m.find('>tbody>tr>td[data-editable], >tbody>tr>td>span>span[data-editable]').off('dblclick.grid-editable',n);m.find('>tbody>tr>td[data-editable], >tbody>tr>td>span>span[data-editable]').on('dblclick.grid-editable',n);$('html').off('click.grid-editable').on('click.grid-editable',function(){if(isMouseIn()){return}else{closeAllEditables()}})})}};mesour.core.createWidget('gridEditable',new r({attributeName:'data-mesour-editable'}));mesour.on.live('mesour-grid-editable',mesour.gridEditable.create)})(jQuery);